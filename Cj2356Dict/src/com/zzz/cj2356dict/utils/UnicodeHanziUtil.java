...
public class UnicodeHanziUtil {
     ...
     * 字符和編碼的映射
     */
    private static Map<String, Integer> nameNumMap = new HashMap<String, Integer>();
    ...
    static {
        ...
        for (String name : nameRangeMap.keySet()) {
            ...
            for (int i = range[0]; i <= range[1]; i++) {
                nameNumMap.put(getStringByUnicode(i), i);
            }
        }
    }
    ...
    public static String getRangeNameByChar(String charStr) {
        ...
        int num = 0;
        for (String str : nameNumMap.keySet()) {
            if (str.equals(charStr)) {
                num = nameNumMap.get(str);
            }
        }
        for (String name : nameRangeMap.keySet()) {
            int[] range = (int[]) nameRangeMap.get(name);
